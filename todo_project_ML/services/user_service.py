"""
회원가입/로그인/토큰재발급/로그아웃의 업무규칙을 담당하는 계층
...
class UserService:
    ...
    def signup(self, body: UserSignUpRequest) -> User:
        ...
        if self.repository.find_by_email(body.email):
            # 이메일 중복 확인 -> 이미 존재하면 409 에러(Conflict)로 거부
            raise HTTPException(
                status.HTTP_409_CONFLICT, detail="이미 사용 중인 이메일입니다."
            )
        # 비밀번호는 반드시 해싱으로 저장 (평문 저장 금지)
    ...
    def refresh(self, refresh_token: str) -> dict:
        """
        access token이 만료되었을 때, 재로그인 없이 새 access_token만 발급받는 기능
