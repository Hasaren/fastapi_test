...
class TodoRepository:
    ...
    def find_by_id(self, todo_id: int, user_id: int) -> Todo | None:
        """
        todo_id와 user_id를 동시에 조건을 걸어서, 나의 Todo가 아니면 조회가 안되게 설정
        찾으면 Todo 객체 반환, 못 찾으면 None을 반환
        """
    ...
    def save(self, todo: Todo) -> Todo:
        """
        새로만든 Todo든, 기존 Todo를 수정하든 상관없이 저장한다.
        session.add() --> 이미 세션이 추적 중인 객체(수정된 기존 Todo)에 대해서는
            아무 일도 하지 않고, 새 객체일 때만 의미가 있다.
        이 단점이 있어도 모두에게 같은 메서드를 사용할 수 있어 코드가 단순해진다.
        """
