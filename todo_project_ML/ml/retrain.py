...
- 이 스크립트도 FastAPI서버와 분리된 별도의 프로세스로 실행한다.
    (수동 실행 -> 이후 필요하면 cron/스케쥴러로 자동화)
...
def load_original_data() -> pd.DataFrame:
    """최초 학습에 사용한 샘플 데이터. 재학습 때도 기반 데이터로 포함시킨다."""
...
def load_user_corrected_data() -> pd.DataFrame:
    ...
    try:
        # final_category가 NULL이 아닌(=사용자가 한번이라도 확인한) 행만 조회
...
def main():
    ...
    # 버전 번호는 train_model.py 가 만들어둔 다음부터 자동으로 이어진다.
    ...
    print(f'[INFO] FastAPI 서버를 재시작 해야 새 모델이 반영됩니다.')
