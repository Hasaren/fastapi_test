...
MLOps 핵심 개념 --> 모델을 배포하고 끝이 아니라 배포 후에도 계속 지켜본다.(모니터링)는 것이
                    이 파일의 존재 이유
                    서서히 정확도가 떨어지는 현상 (model drift)을 감지하기 위해
...
def get_model_accuracy_handler(session=Depends(get_session)):
    ...
    정확도 계산 대상으로 삼는다.
    predicted_category만 있고 final_category가 없는(=아직 아무도 확인 안한) Todo는
    정확도 계산에서 제외된다.
    """
    total_stmt = select(func.count()).select_from(Todo).where(Todo.final_category.is_not(None))  # final_category가 채워진(=사람이 확인한) 개수
    total_labeled = session.execute(total_stmt).scalar_one()

    # predicted_category와 final_category가 "정확히" 일치하는 개수
    # models.py에서 이 두 개의 컬럼을 나누었다.
    correct_stmt = select(func.count()).select_from(Todo).where(
        Todo.final_category.is_not(None),
        Todo.predicted_category == Todo.final_category)# predicted_category == final_category인 개수
    correct = session.execute(correct_stmt).scalar_one()

    # total_labeled가 0이면 0으로 나누는 에러가 발생하므로 미리 방어 처리
