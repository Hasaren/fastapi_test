...
Streamlit도 따로 streamlit run으로 실행해야 한다.
"""
...
""", unsafe_allow_html=True) # unsafe_allow_html=True --> html/css 가 먼저 적용이 되도록 
...
def logout():
    """세션 상태를 비워서 로그아웃 처리 (서버에 요청 보낼 필요 없다 -> jwt는 stateless 방식)"""
    st.session_state.access_token = None
    st.session_state.user_email = None
...
def extract_error_message(res: requests.Response, fallback: str) -> str:
    ...
    - 응답이 JSON이 아니거나 (500에러) 파싱 자체가 실패하는 경우도 대비
    """
    try:
    ...
    if isinstance(detail, list):
        messages = []
        for err in detail:
            # err이 dict라면 msg키의 값으 msg 변수에 넣어라, 아니면 err을 문자열로 형변환해서
            # msg 변수에 넣어라
        ...
        # messages가 있다면 메시지 사이사이에 엔터 삽입, 아니면 그냥 fallback변수에 있는 값이 반환
...
if not st.session_state.access_token:
        ...
        if submitted:
            try:
                ...
                if res.status_code == 200: # 로그인 성공
                    ...
                    st.rerun() # 화면을 즉시 새로고침 해서 아래의 Todo화면으로 전환
                else:
    ...
    st.stop() # 로그인 전이면 아래 코드는 아예 실행되지 않고 멈춘다.
...
if total > 0:
    # st.progress 위젯 : 진행률 표시 막대를 화면에 그려준다.
...
for todo in todos:
    ...
    #=== 카테고리 자동 분류 표시/확정 영역 MLOps 확장 부분===============================================
    predicted = todo.get('predicted_category')
    final = todo.get('final_category')

    # predicted가 None인 경우 (=서버에 모델이 로드 안되어 있을 시점에 생성된 Todo)는
    # 이 영역 자체를 표시하지 않는다.
    if predicted:
        ...
        with cat_col1:
            if final:
                ...
                # 아직 아무도 확인하지 않은, 모델 예측 그대로의 상태
                st.caption(f'모델 예측: **{predicted}** (확인 필요)')

        with cat_col2:
            # 확정 값이 있으면 있는 값을 사용하고, 없으면 예측값을 selectbox의 초기 선택값으로 사용
            current_value = final if final else predicted
            selected = st.selectbox('카테고리 확인/수정',CATEGORY_OPTIONS,
                                    index=CATEGORY_OPTIONS.index(current_value) if current_value in CATEGORY_OPTIONS else 0,
                                    key=f'category_{todo["id"]}',
                                    label_visibility='collapsed', # 레이블을 화면에 안보이게(위 caption이 레이블 역할을 대신 한다.)
            )
