'''
langchain/00_check_setup.py
-----------------------------
설치 확인 + gemini API key 연결 확인 테스트

이 파일의 목적
- .env에 넣어둔 API KEY가 제대로 읽히는지 확인
...
if not api_key: # api_key가 None이거나 없다면 (키 인식 실패) -> 강제로 멈춘다.
    raise SystemExit('GOOGLE_API_KEY가 설정되지 않았습니다. .env 파일을 확인')
...
print('연결 성공! 모델 응답:')
print(response.content[0]['text']) # 실제 답변 텍스트는 content 안에 있다.