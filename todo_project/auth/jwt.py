...
- 수명을 길게 하면 -> 탈취당했을 댸 오래 악용가능하고, 서버가 저장 안 하니 강제로 막을 방법도 없음
그래서 "자주 쓰지만 짧게 사는 access_token" + "가끔쓰지만 오래 살고 DB에서 통제 가능한 refresh_token"으로
역할을 쪼갠 것이다. Access_token은 탈취 되어도 피해가 최대 30분으로 제한되고,
로그인 자체는 Refresh_token 덕분에 7일간 끊기지 않는다.

payload에 들어있는 'type':'access' / 'type':'refresh' 필드가 두 토큰을 구분하는 유일한 표식
이게 없으면 서버가 받은 토큰이 access 용인지 refresh 용인지 구별할 방법이 없어서,
...
def _create_token(user_id: int, token_type: str, expires_delta: timedelta) -> str:
    ...
    이름 앞_(언더스코어)의 역할 --> 이 파일 밖에서는 직접 부르지 말고, 아래 두 함수를 통해서만
    호출해라 라는 파이썬의 관례
    """
    payload = {
        "user_id": user_id,
...
def decode_token(token: str, expected_type: str) -> int:
    ...
    expected_type을 반드시 넘겨받아서, access_token인데 refresh_token자리에 사용했다 같은 오남용을 
    여기서 걸러 낸다.
    """
    try:
