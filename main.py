# ===============================
# main.py
# 역할 : FastAPI 맵 정의 + CRUD 라우터 5개
# 
# API 목록
#   GET    /todos --> 전체 할일 조회
#   GET    /todos/{todo_id} --> 단일 할 일 조회
#   POST   /todos --> 할일 생성
#   PATCH  /todos/{todo_id} --> 할일 수정
...
# GET /todos --> 전체 할 일 조회
@app.get('/todos',
         response_model=list[TodoResponse], # 응답 형식 : TodoResponse
...
def get_todos_handler():
    ...
    try:
        ...
        stmt = select(Todo) # 데이터를 조회하는 쿼리 객체 (아직 db에 접근 안했음)
        # session.execute(stmt) : 쿼리 객체를 실제 데이터베이스에 전달해서 실행
        # scalars().all() : 실행 결과에서 테이블의 각 행에 대응되는 ORM 객체를 추출하여 리스트로 반환
        todos = session.execute(stmt).scalars().all() # 전체 결과 리스트로 반환
        return todos
    finally: # 예외가 발생하든 안하든 무조건 실행되는 코드
        session.close()
...
def delete_todo_handler(todo_id:int):
    ...
    try:
        ...
        todo = session.execute(stmt).scalars().first()
        if todo:
            session.delete(todo) # 삭제 준비
            session.commit() # DB 적용
            return 
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail='todo not found')
    finally:
