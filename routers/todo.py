...
# 라우터(Router)란?
#  - 웹 어플리케이션에서 여러 API를 기능별로 구분해 한곳에 모아두기 위한 단위
#  - 서로 관련된 API 엔드포인트들을 하나의 모듈로 묶을 수 있다. 코드를 기능단위로 정리하고 관리하기 좋다.

# 라우팅(routing)란?
#  - 클라이언트로부터 들어온 요청(HTTP매서드+URL 경로)을 어떤 함수가 처리할지 연결하는 과정
#  - 연결과정을 관리하는 역할
#  - FastAPI에서는 APIRouter클래스를 사용해 연관된 경로들을 하나의 라우터로 묶고, 등록, 기능별로 나눈다.
# 
