...
#       파일 이름이 test_로 시작되어야 한다.
#       함수 이름이 test_로 시작해야 한다.
#       권장사항> testDB도 따로 만들어 준다.
# 
#================================================================================
...
# 2. 테스트 전용 DB 설정
TEST_DATABASE_URL = 'postfresql+psycopg2://postgres:1234@localhost:5432/reviewdb_test'
...
@pytest.fixture(scope='module') # pytest가 자동으로 호출해준다. / 이 파일 전체에서 딱 한번만 실행된다.
def setup_db():
    """
    이 파일의 모든 테스트가 시작되기 전 딱 한번만 실행되는 함수
    yield 위 : 테스트 전 준비(테이블 생성)
    yield 안 : 테스트 후 정리 (테이블 삭제) --> 다음에 pytest 실행시 깨끗한 상태에서 실행 할 수 있도록 
    """
