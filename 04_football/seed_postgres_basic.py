...
# csv 5개를 postgreSQL 데이터베이스에 적재하는 초기화 스크립트

# test_crud.py는 CSV 파일을 직접 읽지 않는다.
# PostgreSQL DB에 이미 들어있는걸 조회해서 테스트 한다.
# 따라서, 테스트를 실행하기 전에 이 파일을 한번 실행해서 CSV 데이터를 Postsql 테이블에 넣어야 한다.

# 실행 할 때 마다 기존 테이블을 drop_all로 삭제하고 다시 만든다.
...
def parse_date(value: str) -> date:
    """
    CSV 에서 읽은 날짜 문자열을 Python date 객체로 변환한다.

        CSV 파일 안에 날짜는 "2024-04-08" 같은 문자열이다.
...
def seed() -> None:
    ...
    with Session(engine) as session:
        ...
        # 2. league 테이블 적재
        #   league는 team에서 참조 하므로 team보다 먼저 넣는다.
        leagues = [
            models.League(
        ...
        # flush() 아직 커밋은 하지 않았지만 현재 세션에 쌓인 INSERT를 DB로 보낸다.
        # player와 league 행이 같은 트랜잭션 안에서 실제 테이블에 먼저 보이게 된다.
        # 뒤에 team, team_player 처럼 외래키를 가진 테이블을 넣을 때 
        # 참조할 부모행이 아직 없다는 오류를 피하고, 실행 순서 파악도 쉽다.
        session.flush()

        # 3. team 테이블 적재
        #   team은 league_id를 통해 league에 속한다.
        teams = [
            models.Team(
        ...
        # 4. teamplayer 테이블 적재
        #   teamplayer는 team과 player의 다대다 관계를 표현하는 중간 테이블
        ...
        # commit()을 호출해야 테이블에 INSERT 가 반영된다.
        session.commit()
    
    print('[seed] Postgresql 초기 데이터 적재 완료')
    print(f'[seed] player:{len(players):,}')
    print(f'[seed] league:{len(leagues):,}')
    print(f'[seed] team:{len(teams):,}')
    print(f'[seed] team_player:{len(team_players):,}')
    print(f'[seed] performance:{len(performances):,}')


if __name__ == "__main__":
    # 이 파일을 직접 실행 했을 때만 seed()를 호출
    seed()