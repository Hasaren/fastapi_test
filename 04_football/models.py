...
from __future__ import annotations # 클래스 안에서 타입을 적을 때, 그 클래스가 파일 아래쪽에 정의되어 있어도 그냥 이름만 쓰는 것이 가능
...
from database import Base # SQLAlchemy 가 관리하는 테이블

class Player(Base):
    """판타지 풋볼에서 선택할 수 있는 선수"""

    __tablename__ = 'player' # 실제 PostgreSQL에서 정한 테이블 이름
    ...
    #     back_populates='player' --> 반대편 (Perfomance 클래스)에서는 이 관계를
    ...
    # -----------------------------------------
    # N:M 관계 (선수 여러명 : 팀 여러 개) 다대다 관계 -> Association object 패턴

    # TeamPlayer 라는 클래스를 정식 ORM 클래스로 만들고, 각각 Player와 TeamPlayer, 
    # Team과 TeamPlayer를 각각 1:N 관계로 연결한 뒤,
    # TeamPlayer를 "연결 기록이 저장된 중간 다리"로 활용
    
    # cascade= 'all, delete-orphan'
    #   선수가 team_player에서 제거되거나, 자체가 삭제되면, 관련된 TeamPlayer(가입 기록)
    ...
    # -----------------------------------------
    # association_proxy
    #     team_players는 TeamPlayer(연결 기록) 객체의 리스트라서 "이 선수가 속한 팀 이름들만 조회"
    #     --> player.team_players[0].team.team_name --> 너무 길다.
    # association_proxy('team_players','team')
    #     예전처럼 player.teams 라고만 써도 Team 객체 리스트를 바로 꺼내 볼 수 있게 해주는 "편의 통로"
    # creator=lambda team: TeamPlayer(team=team)
    #   player.teams.append(어떤 팀)처럼 "쓰기"로 사용할 때, 
    #   내부적으로 어떤 TeamPlayer(team=팀, player=해당 선수)를 자동으로 만들어서 연결해준다.
    # -----------------------------------------
...
class Team(Base):
    ...
    # team.players --> team_players 를 거치지 않고 바로 Player 객체 목록을 꺼내는 통로
...
class TeamPlayer(Base):
    """
    Team과 Player의 다대다 관계를 표현하는 연결(association) 테이블 이면서, 
    ...
    team_id: Mapped[int] = mapped_column(
        ForeignKey('team.team_id'), primary_key=True, index=True # team 테이블 안 team_id
    )

    player_id: Mapped[int] = mapped_column(
        ForeignKey('player.player_id'), primary_key=True, index=True # player 테이블 안 player_id
    )
