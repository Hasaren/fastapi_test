...
#   - select() --> 아직 실행되지 않는 SQL
#   - db.scalars(stmt) 또는 db.execute(stmt)를 호출하는 순간 실제로 실행된다.

# from sqlalchemy import func, select
#   func: COUNT, SUM 처럼 SQL의 집계함수를 파이썬에서 사용할 때
#   select: SELECT 문장을 설계하는 함수(아직 실행 아님)

# from sqlalchemy.orm import Session, joinedload, selectinload
#   Session: DB 와 대화하는 작업공간
#   joinedload: 관계 데이터를 SQL JOIN으로 한번에 같이 가져오는 옵션
#   selectinload: 관계 데이터를 "추가 SELECT한번"으로 따로 가져오는 옵션
# =====================================================
...
def get_players(db: Session, skip: int=0, limit: int=100, 
    ...
    # scalars() --> 각 행에서 첫번째 컬럼(여기서는 Player객체 자체)만 꺼내서 
...
def get_leagues(db: Session, skip: int=0, limit: int=100,
    ...
    """
    리그 목록을 조회, 각 리그에 속한 teams까지 한번에 즉시 로딩(eager load)

    joinedload: SQL JOIN으로 리그과 팀을 "한번의 쿼리"로 같이 가져온다.
    (Lazy loading --> N+1 문제를 해결하기 위함)
    """
...
def get_teams(db: Session, skip: int=0, limit: int=100,
    ...
    """
    팀 목록 조회, league_id 를 주면 특정 리그 소속 팀만 필터링
    ---------------------
    team.team_players --> 이 팀에 연결된 TeamPlayer(가입 목록) 목록 --> 그 TeamPlayer
        각각의 .player는 다시 그 기록에 연결된 Player 객체가 된다.
    ...
    selectinload(models.Team.team_players)
        ...
        별도의 IN 조회를 한번 더 하는 방식)

    .joinedload(models.TeamPlayer.player)
        그 다음 단계 (TeamPlayer -> Player)는 selectinload가 만든 두번째 쿼리 안에서
        JOIN으로 같이 가져온다.

    --> 1단계 selectinload(추가 쿼리 방식), 2단계는 joinedload(같은 쿼리 안에서 JOIN)를
        체이닝 해서 총 2번의 쿼리로 team + team_player + player 정보를 다 채워온다.
    """
