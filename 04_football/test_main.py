...
#     느리고 실수가 생길 수 있다. 이 확인과정을 코드로 자동화
# 2) 회귀 방지
#     오늘 만든 기능이 내일 다른 코드 수정 때문에 망가지는 것을 자동으로 잡아내기 위해
#     pytest 한 줄 치면 수십 개 테스트가 몇 조만에 실행된다. --> 증거로 확인
# 3) TestClient 를 사용하는 이유 "서버를 안 켜도 되니까"
#     진짜 서버를 키려면 포트를 열고, 네트워크 소켓을 열고, 실제로 통신을 해야한다.
#     TestClient는 실제 네트워크 통신 없이 main.py의 app 객체를 파이썬 함수 호출 수준
#     에서 그대로 실행 --> 서버를 따로 켜둘 필요가 없다.
...
def test_read_players_by_name():
    ...
    # assert : 합격/불합격을 판정하는 채점 기준이라 생각하자
...
def test_read_leagues_with_id():
    ...
    # response.json()['teams'] 
    # schemas.League.teams: List[TeamBase] 필드
...
def test_read_teams_for_one_league():
    ...
    # 전체 20개 팀 중 league_id = 5001 소속인 12개만 응답에 남아야 한다.
