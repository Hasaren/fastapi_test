...
api_description = '''이 API는 Sports World Central(SWC) 판타지 풋볼 API의 정보를 읽기 전용으로 제공합니다. 
...
@app.get('/',
        ...
        #  response_description : 이 API를 호출하면 어떤 응답이 나오는 가에 대한 설명 문구
         response_description='''메시지가 포함된 JSON 레코드 입니다.
         API가 실행 중이면 성공 메시지를 반환합니다.''',
        #  operation_id : 이 엔드포인트를 가리키는 전역 고유 이름. URL(/)과
        # 메서드(GET) 조합만으로는 사람이 읽기 힘들 수 있는데, operation_id를 지정해주면
        # OpenAPI 명세를 읽어 이 이름을 함수명으로 그대로 사용한다.
...
@app.get('/v0/players/', response_model=list[schemas.Player],
         ...
         선수 수를 셀 때는 player_id 값을 사용하지 않습니다.
         ID는 순차적으로 진행되지 않습니다.''',
...
@app.get('/v0/players/{player_id}', response_model=schemas.Player,
         ...
         description='''다른 API 호출(v0_get_players 등)에서 얻는 SWC 선수 ID를 이용해
         해당 선수를 조회할 수 있습니다.''',
...
@app.get('/v0/performances/', response_model=list[schemas.Performance],
         ...
         Performance ID는 내부 ID 이므로 개수 계산이나 로직에 사용하지 않습니다.
         순차적으로 사용되지 않습니다.''',
...
@app.get('/v0/leagues/', response_model=list[schemas.League],
         ...
         skip과 limit를 사용해 페이징을 사용합니다. 리그 이름은 고유하지 않습니다.
         League ID는 내부 ID이므로 개수 계산이나 로직에 사용하지 않습니다.
         순차적으로 보여지지 않습니다.''',
...
@app.get('/v0/teams/', response_model=list[schemas.Team],
         ...
         다른 쿼리에서 얻는 Team ID를 이 쿼리의 Team ID와 매칭할 수 있습니다.
...
def read_teams(skip: int=Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'), 
                limit: int=Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'),
                minimum_last_changed_date: date = Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.'),
               team_name: str=Query(None, description='반환할 팀 이름입니다. SWC에서 고유하지 않으나, 리그 내부에서 고유합니다.'),
               league_id: int=Query(None, description='반환할 팀이 속한 리그 id입니다. SWC에서 고유합니다.'),
               db: Session= Depends(get_db)):
    """팀 목록을 조회한다.
    리그 id로 특정 리그의 팀만 가져올 수 있다.
    """
...
@app.get('/v0/counts/', response_model=schemas.Counts,
         ...
         팀, 선수 수를 확인합니다. 다른 조회 엔드포인트의 skip과 limit 매개변수와 함께
         사용합니다. 개수를 확인하려면 다른 API 대신 이 엔드포인트를 사용합니다.''',
         response_description='SWC 판타지 풋볼의 리그, 팀, 선수 수입니다.',
         operation_id='v0_counts',
         tags=['membership'])
