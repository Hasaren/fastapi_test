...
def register_book(isbn: str=Form(...), image: UploadFile=File(...), db: Session=Depends(get_db)):
    ...
    if result.status == 'invalid_image':
        # 415에러: Unsupported Media Type (파일 형식이 이미지가 아니다. 지원하지 않는 미디어 타입)
...
def ui_lookup_form(request: Request):
    '''
    request: Request --> jinja2templates가 화면을 만들 때 반드시 필요로 하는 매개 변수
                        내부적으로 어떤 요청이 어떤 서버/포트로 왔는지 등을 알아야
                        url_for() 같은 기능이 동작한다.

    templates.TemplateResponse(...) --> context에 담긴 데이터를 채워 넣은 뒤,
                                    완성된 html을 브라우저에 응답으로 보낸다.
    context={} --> 맨 처음 화면 진입시 조회 결과가 없으므로
    '''
...
def ui_lookup_submit(request: Request, isbn: str=Form(...), db: Session=Depends(get_db)):
    ...
    return templates.TemplateResponse(
        ...
        context={'result': result, 'isbn': isbn},
    )
...
def shelf(request: Request, db: Session=Depends(get_db)):
    ...
    shelf_items = []
    for book in books:
        cover_url = None # '책 표지가 없다'로 미리 설정
        if book.cover_path:
            cover_url = f'/uploads/{Path(book.cover_path).name}'
