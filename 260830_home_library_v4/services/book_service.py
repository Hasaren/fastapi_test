...
from models import Book # models.py에서 정의한 Book 클래스
...
class ServiceResult:
    ...
    HTML 라우터 --> 실패 시 에러 화면이 아니라, "이미 등록된 책 입니다" 와 같은 안내문구가 나와야 한다.
...
def lookup_book_service(isbn: str, db: Session) -> ServiceResult:
    """
    isbn 만으로 국립중앙도서관 서지정보를 조회해서 책을 등록하는 함수 (main.py의 lookup_book 함수를 옮겼다.)
    """
    # 1단계: ISBN형식이 올바른지 체크섬으로 검증
    ...
    if existing_book:
        # 참이라면 None이 아니라 실제 Book객체가 있다는 뜻 --> 이미 있는 책
    ...
    book = Book(
        ...
        cover_path = None, # 표지 사진 없이 ISBN으로 등록되는 기능
        recognition_status='confirmed', # 국립중앙도서관의 정식 데이터라서 '확정'으로 표시   
    )
    db.add(book) # 이 책을 저장 대기열에 올려라. --> 아직 DB에 실제로 쓰이지는 않았다.
    db.commit() # 대기열에 있는 책을 DB에 확정 저장해라. --> SQL INSERT 실행.
    db.refresh(book) # DB가 자동 생성한 값(id, create_at)을 book 객체에 다시 채워 넣는다.
...
def register_book_service(isbn: str, raw_image: bytes, original_filename: str, db: Session) -> ServiceResult:
    """
    ISBN + 책 표지 사진을 함께 등록하는 함수 (main.py 의 register_book 함수의 로직을 따왔다.)

    1단계와 2단계는 lookup_book_service와 동일한 패턴
    """
    # 1단계: ISBN형식이 올바른지 체크섬으로 검증
    validated_isbn = normalize_isbn(isbn)
    if not validated_isbn:
    ...
    path.write_bytes(raw_image) # 실제로 디스크에 이미지 파일을 저장한다.
    ...
    if metadata:
        ...
        # 서지 정보를 못찾아도 표지 사진과 ISBN이 있다면 일단 등록한다. 수동 등록
        title = f'수동등록: ISBN {validated_isbn} (서지 정보 조회 실패)'
        author = None
        publisher = None
    ...
    book = Book(
        ...
        author = author,
        publisher = publisher,
        cover_path = str(path), # DB는 path 객체 모른다. 문자열만 이해
