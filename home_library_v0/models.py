...
# sqlalchemy 의 String --> 최대 길이를 지정해야 한다. 짧고 정해진 범위의 문자열에 적합하다.
#           DB 쪽에서 길이 초과 시 에러가 나서 실수로 너무 긴 값이 들어가는 것을 막아주는 안전장치 역할
# sqlalchemy 의 Text --> 길이 제한이 없다. DB가 허용하는 최대치까지 
...
# ---------------------------
class Book(Base):
    __tablename__= 'books' # 실제 DB에 저장될 테이블 이름

    id: Mapped[int] = mapped_column(primary_key=True) # 기본키
    # ISBM은 OCR 인식이 실패할 수도 있다. 모를 수도 있으니 None 허용
    isbm: Mapped[str|None] = mapped_column(
        String(13),
        unique=True, # 중복 방지
    ...
    # 사진 파일 자체는 DB에 넣지않고 파일 시스템에 두고 경로만 기록
    cover_path: Mapped[str|None] = mapped_column(String(500))
    # OCR 검증이 성공했는지 실패했는지 상태를 나타내는 문자열
    # default='confirmed' --> 기본값은 'confirmed'(확정)/ need_review(확인필요)
    recognition_status: Mapped[str] = mapped_column(String(20), default='confirmed')
    # 이 책이 언제 등록되었는지 자동 등록
    # default=datetime.utc --> 저장되는 순간의 시각을 자동으로 채워준다.
...
class ReadingStatus(Base):
    ...
    current_page: Mapped[int] = mapped_column(Integer, default=0) # 현재 읽은 쪽 수, 기본값 0
...
class Review(Base):
    ...
    content: Mapped[str] = mapped_column(Text) # 리뷰의 본문
