...
# Streamlit 이 이 주소로 HTTP 요청을 보내서 데이터를 주고 받는다.
# FastAPI와 Streamlit은 완전히 다른 프로그램 두 개가 따로 실행되는 것 --> 서로 requests로 대화를 하는 구조
...
#     # files={'image':(파일명, 파일 내용 바이트, 파일 타입)} --> FastAPI에 uploadFile 매개변수 image와 일치해야 한다.
#     r = requests.post(
#         f'{API}/books/scan',
...
# --- and image없애고, 버튼 클릭 안쪽에서 image 유무를 명시적으로 분기 ----
if st.button('등록'):
    if image is None:
        # 사진이 없는데 등록을 눌렀을 때 --> 경고만 띄운다.
