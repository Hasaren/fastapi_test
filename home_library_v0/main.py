...
# ===================================================
# POST /books/scan --> 사진을 업로드 해서 책 한 권을 등록하는 API 
...
def scan_book(image: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        #                       디스크에 진짜 파일을 만들지 않고도 파일인 것 처럼 열어볼 수 있다.
        # Image.open(...) --> PIL 라이브러리로 이 바이트가 이미지로 열리는지 시도
        # with ... as probe --> 확인이 끝나면 자동으로 메모리를 정리해준다. (파일을 닫는다.)
        with Image.open(io.BytesIO(raw)) as probe:
            # verify() --> 이 파일이 진짜로 손상되지 않은 이미지 형식이 맞는지 내부적으로 검사
            probe.verify()

    # UnidentifiedImageError --> PIL이 아예 이미지 형식으로조차 인식하지 못했을 때 발생하는 에러
    # OSError --> 파일이 중간에 잘렸거나, 손상된 경우 등 좀 더 넓은 범위의 파일 관련 에러
    except (UnidentifiedImageError, OSError):
        # HTTP 상태코드 415(지원하지 않는 파일 형식) 함수 실행 중단 시킨다.
        # raise 를 만나는 순간 DB등록은 실행되지 않는다.
        raise HTTPException(415, '올바른 이미지가 아닙니다.')
    ...
    status_value = 'confirmed' if isbn else 'needs_riview'


    book = Book(title=title, isbn=isbn, cover_path=str(path), recognition_status=status_value)

    db.add(book) # 이 책 데이터를 저장 대기열에 올린다. (DB에 실제로 사용되지는 않는다.)
    db.commit() # 대기열에 올린 내용을 실제로 DB에 확정 저장한다.
    db.refresh(book) # DB가 자동으로 채운 값(id, created_at)을 book객체에 다시 불러와 채워준다.

    return book # book 객체를 자동으로 JSON으로 변환해서 Streamlit에게 되돌려 준다.

# GET /books --> 등록된 책 전체 목록을 돌려주는 API
@app.get('/books', status_code=status.HTTP_200_OK)
def list_books(db:Session = Depends(get_db)):
    # select(Book) --> books 테이블에서 모두 가져와라. SQL SELECT 문을 파이썬 코드로 작성
    # db.scalars(...).all() --> SELECT 문을 실제로 DB에 실행시키고, 결과를 파이썬 리스트로 받는다.
